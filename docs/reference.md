...
Auto-generated by `/plans sync`. Never hand-edit. Consumed by `roadmap.html` and any future tooling.
...
plans/active/*.md  +  plans/shipped/*.md
        ...
        |  extract frontmatter
        v
plans/plans.json   (regenerated, superseded plans excluded)
        |
...
Guided creation of a new plan file. Asks: name, type, priority, depends_on, blocks, owner. Writes `plans/active/NAME.md` with correct frontmatter and status banner template. Reminds the user to add a row to `plans/STATUS.md`. Does not set `in_flight: true`.
