...
If your assistant is Claude Code, a Stop hook watches the end of each session. When the session changed code, a plan is marked `in_flight: true`, and no file under `plans/` was touched, it asks once for the plan update before the session ends. Update the covering plan's `## Status` banner and `last_updated`, or say that no plan covers the work: either answer satisfies it, and it never asks twice in the same turn. To turn it off, remove the plans entries from `.claude/settings.json`.
